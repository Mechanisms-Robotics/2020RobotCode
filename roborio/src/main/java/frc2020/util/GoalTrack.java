...
public class GoalTrack {
    ...
    public synchronized boolean tryUpdate(double timestamp, Pose2d new_observation) {
        ...
        double distance = mSmoothedPosition.minus(new_observation).getTranslation().getNorm();
        if (distance < Constants.MAX_TRACK_DISTANCE) {
    ...
    synchronized void pruneByTime() {
        double delete_before = Timer.getFPGATimestamp() - Constants.MAX_TRACK_AGE;
    ...
    synchronized void smooth() {
        if (isAlive()) {
            ...
            for (Map.Entry<Double, Pose2d> entry : mObservedPositions.entrySet()) {
                if (t_now - entry.getKey() > Constants.MAX_GOAL_TRACK_SMOOTHING_TIME) {
                    continue;
                }
                ++num_samples;
                x += entry.getValue().getTranslation().getX();
                y += entry.getValue().getTranslation().getY();
                c += entry.getValue().getRotation().getCos();
                s += entry.getValue().getRotation().getSin();
            }
            x /= num_samples;
            y /= num_samples;
            s /= num_samples;
            ...
            if (num_samples == 0) {
                ...
                mSmoothedPosition = new Pose2d(x, y, new Rotation2d(c, s));
            }
        }
    }
    ...
    public synchronized double getStability() {
        return Math.min(1.0, mObservedPositions.size() / (Constants.CAMERA_FRAME_RATE * Constants.MAX_TRACK_AGE));
    }
